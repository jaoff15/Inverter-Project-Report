...
This results in, that the total temperature rise from the thermal paste to the MOSFET junction can be calculated with equation \ref{eq:tempMOSFET}.

    \begin{equation}
        T_{JP} = P_{loss} \cdot (R_{JC} + R_{PCB})
    \end{equation}
    
Where $R_{JC}$ is the internal