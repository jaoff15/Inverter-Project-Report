...
Where the $T_A$ is the ambient temperature. It has to be noticed that the temperature difference over the heat sink is depending on the total power loss, and the temperature difference from the paste to the junction of a MOSFET is depending on the power loss for one MOSFET. 
Due to the thermal resistance of the heat sink is common for all MOSFET's and there is a parallel thermal resistance for each MOSFET from the paste to the junction.
