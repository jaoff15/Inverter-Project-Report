...
One of the most important and useful motor drives in modern electronics is the BLDC and PMAC (permanent magnet motor). The motor achives this status by being highly versatile, robust and have high torque. The project focuses on the controlling of a ME1117 Brush less Permanent Magnet Motor, for an electrical go kart. \\

The project aims to design and build the hardware shown in the green square as seen on figure \ref{fig:Possiblesolution}. The figure show a simplified motor controller consisting of a three-phase inverter controlled by a Zynq-7000 controller.
...
The illustration shows how the main connections between elements could look. On the right is the motor itself with an attached encoder to measure the rotor angle. The motor is driven by the three-phase inverter developed in this project as can be seen in the middle. Below the inverter in the middle is the Zynq controller on a Zybo training board which controls the inverter. By receiving a range of sensor signals the controller handles the control needed for the inverter to run the motor at the requested torque.
